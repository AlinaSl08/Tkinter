...
def notepad_exit():
    ...
    if answer:
        root.destroy() #выход из программы
...
file_menu = Menu(main_menu, tearoff=0) #tearoff=0 - бирает линию, которая выводит в отдельное окно менюшку
...
text_fild = Text(f_text,
                 ...
                 padx=10,
                 pady=10,
                 wrap=WORD, #wrap=WORD - делает корректный перевод слов на другую строку
                 insertbackground="brown", #делает курсор ввода(полосочку) видимой и цвет определяет
                 selectbackground="#8D917A", #цвет выделения текста
                 spacing3=10, #делает абзацы
                 width=30,
                 font="Arial 10 bold"
                 )
...
scroll = Scrollbar(f_text, command=text_fild.yview) #делает скролл
